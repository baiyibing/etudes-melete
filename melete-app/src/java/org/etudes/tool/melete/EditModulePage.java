/**********************************************************************************
 *
 * $URL: https://source.etudes.org/svn/apps/melete/trunk/melete-app/src/java/org/etudes/tool/melete/EditModulePage.java $
 * $Id: EditModulePage.java 10493 2015-04-17 19:50:54Z mallikamt $
 ***********************************************************************************
 *
...
public class EditModulePage extends ModulePage implements Serializable/* , ToolBean */
{
	...
	private SectionObjService prevSection = null;
	private ModuleDateBeanService prevMdBean = null;
	private ModuleDateBeanService nextMdBean = null;
	...
	public String cancel()
	{
		...
		if (mPage.getNavigateView())
		{
			try
			{
				int moduleId = module.getModuleId();
				CourseModuleService cm = moduleService.getCourseModule(moduleId, mPage.getCourse_id());
				
				context.getExternalContext().redirect("view_module.jsf?modId=" + module.getModuleId() + "&modSeqNo=" + cm.getSeqNo());
			}
			catch (Exception e)
			{
				return null;
		...
		this.prevMdBean = null;
		this.nextMdBean = null;
		this.module = null;
	...
	public void setEditInfo(ModuleDateBean mdbean)
	{
		...
		int currSeqNo = module.getCoursemodule().getSeqNo();
		...
		if ((mdbean.getSectionBeans() == null) || (mdbean.getSectionBeans().isEmpty()))
		{
			...
			int nextSeqNo = moduleService.getAuthNextSeqNo(userId, courseId, currSeqNo);
			if (nextSeqNo <= 0)
			{
				setNextMdBean(null);
			...
			{
				ModuleDateBean nextMdBean = (ModuleDateBean) moduleService.getModuleDateBeanBySeq(userId, courseId, nextSeqNo);
				setNextMdBean(nextMdBean);
				setHasNextItem(true);
			}
		}
		else
		...
		int prevSeqNo = moduleService.getAuthPrevSeqNo(userId, courseId, currSeqNo);
		if (prevSeqNo <= 0)
		{
			setHasPrevItem(false);
		}
		else
		{
			setHasPrevItem(true);
			ModuleDateBean prevMdBean = (ModuleDateBean) moduleService.getModuleDateBeanBySeq(userId, courseId, prevSeqNo);
			setPrevMdBean(prevMdBean);
			if (prevMdBean.getSectionBeans() == null || prevMdBean.getSectionBeans().isEmpty())
			{
				setPrevSection(null);
			}
			else
			{
				setPrevSection(((SectionBeanService) prevMdBean.getSectionBeans().get(prevMdBean.getSectionBeans().size() - 1)).getSection());
			}
		}
	}
	...
	public void editNextItemListener(ActionEvent evt)
	{
		callFromAddContent = false;
		...
		else
		{
			...
			try
			{
				context.getExternalContext().redirect("edit_module.jsf?editmodid=" + nextMdBean.getModuleId());
			}
			catch (Exception e)
			{
				return;
	 ...
	 * @param evt
	 */
	public void editPrevItemListener(ActionEvent evt)
	{
		callFromAddContent = false;
		...
		if (prevSection != null)
		{
			try
			{
				context.getExternalContext().redirect("editmodulesections.jsf?sectionId=" + prevSection.getSectionId().toString());
			}
			catch (Exception e)
			{
				return;
		...
		else
		{
			...
			try
			{
				context.getExternalContext().redirect("edit_module.jsf?editmodid=" + prevMdBean.getModuleId());
			}
			catch (Exception e)
			{
				return;
	...
	public SectionObjService getPrevSection()
	{
	...
	/**
	 * @param nextMdBean
	 *        the nextMdBean to set
	 */
	public void setNextMdBean(ModuleDateBeanService nextMdBean)
	{
		this.nextMdBean = nextMdBean;
	}
	
	public ModuleDateBeanService getNextMdBean()
	{
		return this.nextMdBean;
	}	
	
	/**
	 * @param prevMdBean
	 *        the prevMdBean to set
	 */
	...
	/**
	 * @param prevSection
	 *        the prevSection to set
	 */
	public void setPrevSection(SectionObjService prevSection)
	{
