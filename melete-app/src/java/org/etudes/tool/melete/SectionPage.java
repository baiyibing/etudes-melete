/**********************************************************************************
 *
 * $URL: https://source.etudes.org/svn/apps/melete/trunk/melete-app/src/java/org/etudes/tool/melete/SectionPage.java $
 * $$
 * $Id: SectionPage.java 12127 2015-11-25 15:10:28Z mallikamt $
 ************************************************************************************
 *
...
public abstract class SectionPage implements Serializable
{
	...
	{
		this.linkUrl = (linkUrl != null) ? linkUrl.trim() : linkUrl;	
	}
	...
	{
		this.secResourceDescription = (secResourceDescription != null) ? secResourceDescription.trim() : secResourceDescription;
	}
	...
	{
		this.secResourceName = (secResourceName != null) ? secResourceName.trim() : secResourceName;
	}
		...
		previewContentData = "";
			...
			if (meleteResource != null && meleteResource.getResourceId() != null && meleteResource.getResourceId().trim().length() > 0)
			{
				if (this.section.getContentType().equals("typeEditor"))
				{
					...
					if (Util.FindNestedHTMLTags(contentEditor) || Util.FindFilmsOnDemand(contentEditor))
					{
						this.previewContentData = getMeleteCHService().getResourceUrl(meleteResource.getResourceId());
						contentWithHtml = true;
					}
				}
				else if (this.section.getContentType().equals("typeLink"))
				{
					byte[] previewContents = getMeleteCHService().getResource(meleteResource.getResourceId()).getContent();
					if (previewContents != null) this.previewContentData = new String(previewContents, "UTF-8");
					this.previewContentData = previewContentData.trim();
				}
				else
				{
					this.previewContentData = getMeleteCHService().getResourceUrl(meleteResource.getResourceId());
	...
	{
		this.currLinkUrl = (currLinkUrl != null) ? currLinkUrl.trim() : currLinkUrl;
	}
