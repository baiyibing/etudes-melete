/**********************************************************************************
 *
 * $URL: https://source.etudes.org/svn/apps/melete/trunk/melete-app/src/java/org/etudes/tool/melete/EditSectionPage.java $
 * $Id: EditSectionPage.java 12127 2015-11-25 15:10:28Z mallikamt $
 ***********************************************************************************
...
public class EditSectionPage extends SectionPage implements Serializable
{
	...
	private Boolean hasNextModule = true;
	...
	public String setEditInfo(SectionObjService section1)
	{
		...
		try
		{
			...
			hasNextModule = moduleService.getAuthNextSeqNo(getCurrUserId(), getCurrentCourseId(), module.getCoursemodule().getSeqNo()) > 0 ? true : false;
		}
		catch (Exception e)
		{
	...
	public ModuleDateBeanService getMdBean()
	{
		if (mdBean == null)
		{
			...
			mdBean = moduleService.getModuleDateBean(userId, courseId, this.section.getModuleId());
		}
		if (mdBean == null) logger.info("Edit Section Page : getMdBean method fails to get module for the section" + section);
	...
	{
		return sectionService.getSectionDisplaySequence(this.section);
	}	
						...
						else
						{
									...
									&& meleteResource.getResourceId().trim().length() != 0)
							{
								...
								if (section.getContentType().equals("typeLink")
										&& (secResourceName != null && secResourceName.trim().endsWith(".")))
									secResourceName = secResourceName.substring(0, secResourceName.length() - 1);
...
//		 editId = (String) evt.getComponent().getAttributes().get("sectionId");
	...
	public String Finish()
	{
		...
		if (mPage.getNavigateView())
		{
			try
			{
				context.getExternalContext().redirect("view_section.jsf?moduleId="+section.getModuleId()+"&sectionId="+section.getSectionId()+"&moduleSeqNo="+module.getCoursemodule().getSeqNo());
				return "view_section";
			}
			catch (Exception e)
			{
	...
	public boolean isHasNextModule()
	{
		...
		if (hasNextModule == null)
		{
			hasNextModule = false;
			try
			{
				nextSeqNo  = moduleService.getAuthNextSeqNo(getCurrUserId(), getCurrentCourseId(), module.getCoursemodule().getSeqNo());
				if (nextSeqNo > 0) hasNextModule = true;
			}
			catch (Exception e)
			{
			}
		}
		return hasNextModule.booleanValue();
	}	
	 ...
	 * @param evt
	 */
	public void editNextModuleListener(ActionEvent evt)
	{
		if(saveHere().equals("failure"))
		{
			return;
		...
		int nextSeqNo  = moduleService.getAuthNextSeqNo(getCurrUserId(), getCurrentCourseId(), module.getCoursemodule().getSeqNo());
		if (nextSeqNo > 0)
		{
			ValueBinding binding = Util.getBinding("#{editModulePage}");
			ModuleDateBean mdbean = (ModuleDateBean) moduleService.getModuleDateBeanBySeq(getCurrUserId(), getCurrentCourseId(), nextSeqNo);
			EditModulePage emPage = (EditModulePage) binding.getValue(ctx);
			emPage.resetModuleNumber();
			try
			{
				FacesContext.getCurrentInstance().getExternalContext().redirect("edit_module.jsf?editmodid=" + mdbean.getModuleId());
			}
			catch (Exception e)
			{
				e.getMessage();
	 ...
	 * @param evt
	 */
	public void editModuleListener(ActionEvent evt)
	{
		if (saveHere().equals("failure"))
		{
			return;
		...
		ValueBinding binding = Util.getBinding("#{editModulePage}");
		ModuleDateBean mdbean = (ModuleDateBean) moduleService.getModuleDateBeanBySeq(getCurrUserId(), getCurrentCourseId(), module.getCoursemodule()
				.getSeqNo());
		EditModulePage emPage = (EditModulePage) binding.getValue(ctx);
		emPage.resetModuleNumber();
		try
