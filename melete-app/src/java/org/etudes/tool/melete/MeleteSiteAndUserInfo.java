/**********************************************************************************
 *
 * $URL: https://source.etudes.org/svn/apps/melete/trunk/melete-app/src/java/org/etudes/tool/melete/MeleteSiteAndUserInfo.java $
 * $Id: MeleteSiteAndUserInfo.java 10493 2015-04-17 19:50:54Z mallikamt $  
 ***********************************************************************************
 *
 * Copyright (c) 2008, 2011, 2015 Etudes, Inc.
 *
...
public class MeleteSiteAndUserInfo
{
	...
	public boolean checkMeleteRights()
	{
		try
		{
			if (isSuperUser() || isUserAuthor() || isUserStudent()  || isUserSiteObserver())
			{
				return true;
			}
			else
	...
	/**
	 * Check if the current user has permission as site observer.
	 * 
	 * @return true if the current user has permission to perform this action, false if not.
	 */
	public boolean isUserSiteObserver() throws Exception
	{
		try
		{
			return meleteSecurityService.allowObserver(ToolManager.getCurrentPlacement().getContext());
		}
		catch (Exception e)
		{
			throw e;
	...
	public void setNavigateView(boolean goView)
	{
		FacesContext ctx = FacesContext.getCurrentInstance();
		Map m = ctx.getExternalContext().getSessionMap();
		if (goView)
			m.put("goView", new Boolean(goView));
		else
			m.remove("goView");
	}
	
	public boolean getNavigateView()
	{
		FacesContext ctx = FacesContext.getCurrentInstance();
		Map m = ctx.getExternalContext().getSessionMap();
		if (m.containsKey("goView")) return true;
		return false;
	}
