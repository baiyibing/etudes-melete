/**********************************************************************************
 *
 * $URL: https://source.etudes.org/svn/apps/melete/trunk/melete-impl/src/java/org/etudes/component/app/melete/ModuleDB.java $
 * $Id: ModuleDB.java 12127 2015-11-25 15:10:28Z mallikamt $
 ***********************************************************************************
 *
...
public class ModuleDB implements Serializable
{
	...
	/**
	 * Check to see if there is Observer access to course
	 *
	 * @param user_id
	 *        User id -- Not in use
	 * @param course_id
	 ...
	 * @return
	 */
	public boolean checkObserverAccess(String user_id, String course_id)
	{
		try
		{
			return meleteSecurityService.allowObserver(course_id);
		}
		catch (Exception e)
		{
			return false;
	...
	public int getNextSeqNo(String userId, String courseId, int currSeqNo)
	{
		...
		try
		{
			if (meleteSecurityService.allowAuthor(courseId) || meleteSecurityService.allowObserver(courseId))
			{
				queryStr = "select cm.seqNo, count(s.moduleId) from CourseModule cm, Section s, ModuleShdates ms where cm.courseId =:courseId and cm.deleteFlag=0 and cm.archvFlag=0 and cm.seqNo > :currSeqNo and cm.moduleId = s.moduleId and cm.moduleId=ms.moduleId and ms.hideUntilStart=0 and (ms.startDate is null or ms.endDate is null or ms.startDate < ms.endDate) and (ms.startDate is null or ms.allowUntilDate is null or ms.startDate < ms.allowUntilDate) and (ms.endDate is null or ms.allowUntilDate is null or ms.endDate <= ms.allowUntilDate) group by cm.seqNo order by cm.seqNo asc";
			}
			else if (meleteSecurityService.allowStudent(courseId))
			{
	...
	/**
	 * Get next sequence number (after this module in the course). Only checks to ensure module is not deleted or archived
	 * Does not skip invalid modules.
	 *
	 * @param userId
	 *        User id
	 * @param courseId
	 ...
	 * @return Next seq no
	 */
	public int getAuthNextSeqNo(String userId, String courseId, int currSeqNo)
	{
		int nextSeqNo = -1;
		String queryStr = null;
		try
		{
			if (meleteSecurityService.allowAuthor(courseId) || meleteSecurityService.allowObserver(courseId))
			{
				queryStr = "select cm.seqNo from CourseModule cm where cm.courseId =:courseId and cm.deleteFlag=0 and cm.archvFlag=0 and cm.seqNo > :currSeqNo order by cm.seqNo asc";
			}
			else if (meleteSecurityService.allowStudent(courseId))
			{
				return -1;
		...
		try
		{
			...
			q.setParameter("currSeqNo", currSeqNo);
			List res = q.list();
			if (res == null || res.size() == 0) return -1;
			minsequence = (Integer)res.get(0);
	...
	/**
	 * Get prev sequence number (before this module in the course) as long as it is not deleted or archived
	 * It does not skip invalid modules.
	 *
	 * @param userId
	 * @param courseId
	 * @param currSeqNo
	 * @return
	 */
	public int getAuthPrevSeqNo(String userId, String courseId, int currSeqNo)
	{
		int prevSeqNo = -1;
		String queryStr = null;
		boolean allowStudent = false;
		try
		{
			if (meleteSecurityService.allowAuthor(courseId) || meleteSecurityService.allowObserver(courseId))
			{
				queryStr = "select cm.seqNo from CourseModule cm where cm.courseId =:courseId and cm.deleteFlag=0 and cm.archvFlag=0 and cm.seqNo < :currSeqNo order by cm.seqNo desc";
			}
			else if (meleteSecurityService.allowStudent(courseId))
			{
				return -1;
	...
	{
		return getViewModules(userId, courseId, filtered, false);
	}
	 ...
	 *        flag false value means invalid modules need to be picked up
	 * @param callFromCM
	 *        Call is from CourseMap of students
	 * @return list of modules
	 * @throws HibernateException
	 */
	protected List<ViewModBeanService> getViewModules(String userId, String courseId, boolean filtered, boolean callFromCM) throws Exception
	{
		Connection dbConnection = null;
		...
		try
		{
			...
			if (rs != null)
			{
				...
				while (rs.next())
				{
					...
					if (sectionId != 0)
					{
						if (vsBeanMap == null || (moduleId != prevModId)) vsBeanMap = new LinkedHashMap<Integer, ViewSecBean>();
					...
					if ((prevModId == 0) || (moduleId != prevModId))
					{
						...
						// If filtered, do not add hidden modules unless call is from CM
						if (filtered && (!vmBean.isDateFlag() || vsBeanMap == null || vsBeanMap.size() <= 0))
						{
							continue;
						}
						if (filtered && !callFromCM && vmBean.isHiddenTillStart())
						{
							//Do nothing, but exit the if and execute the prevModId code below
						}
						else
						{
						resList.add(vmBean);
...
else
{
	...
	protected ViewModBean populateVmBean(ResultSet rs, Map<Integer, AccessDates> accMap, String courseId) throws SQLException
	{
		...
		boolean hideUntilStart = rs.getBoolean("hide_until_start");
		...
		if ((accMap != null) && (accMap.size() > 0))
		{
			...
			if (ad != null)
			{
				...
				if (ad.overrideHideUntilStart) hideUntilStart = ad.isHideUntilStart();
			}
		}
		...
		vmBean.setHideUntilStart(hideUntilStart);
		return vmBean;
	}
	 ...
	 * @throws HibernateException
	 */
	public List<ViewModBeanService> getCMViewModulesAndDates(String userId, String courseId, boolean filtered) throws HibernateException
	{
		...
		try
		{
			modList = getViewModules(userId, courseId, filtered, true);
		}
		catch (Exception e)
		{
			logger.error(e.toString());
				...
				if (xmlSecList != null)
				{
					for (ListIterator<SecLevelObj> k = xmlSecList.listIterator(); k.hasNext();)
					{
						...
						if (slObj != null)
						{
							...
							if (resource != null && sec.getContentType().equals("typeLink"))
							{
								byte[] data = resource.getContent();
								if (data != null && data.length != 0)
								{
									String printData = new String(data);
									printText.append("<a href=\"" + printData + "\" target=\"_blank\">");
							...
							if (resource != null
									&& (sec.getContentType().equals("typeUpload") || sec.getContentType().equals("typeLTI")))
							{
		...
		try
		{
			...
			try
			{
				...
				for (int i = 0; i < restoreModules.size(); i++)
				{
					...
					moduleShdate.setHideUntilStart(false);
...
//					session.saveOrUpdate(module);
	...
	private boolean shouldUpdateModuleShDateObject(ModuleShdates obj1, ModuleShdates obj2)
	{
		...
		if (obj1.getHideUntilStart() == null)
		{
			if (obj2.getHideUntilStart() != null) return false;
		}
		else if (!obj1.getHideUntilStart().equals(obj2.getHideUntilStart())) return false;
		return true;
	}
		...
		if ((moduleDateBeans != null) && (moduleDateBeans.size() > 0)) {
			for (ListIterator i = moduleDateBeans.listIterator(); i.hasNext();) {
				...
				try {
					...
					mDate.setHideUntilStart(checkModuleDates.getHideUntilStart());
	...
	private Map<Integer, AccessDates> getAccessRecords(String userId,
		...
		String users;
		if (meleteSecurityService.allowStudent(courseId) || meleteSecurityService.allowAuthor(courseId)) {
			// String sql =
			...
			String sql = "select a.users,a.module_id,a.start_date,a.end_date,a.allowuntil_date,a.hide_until_start,a.override_start,a.override_end,a.override_allowuntil,a.override_hide_until_start from melete_special_access a,melete_course_module c where (a.start_date is NULL or a.end_date is NULL or a.start_date < a.end_date) and (a.start_date is NULL or a.allowuntil_date is NULL or a.start_date < a.allowuntil_date) and (a.end_date is NULL or a.allowuntil_date is NULL or a.end_date <= a.allowuntil_date) and a.module_id=c.module_id and c.course_id = ?";
	...
	private int getStudentNavSeqNo(String userId, String courseId, int currSeqNo, boolean prevFlag)
	{
        ...
        try
		{
			...
			if (prevFlag)
			{
				sql = "select cm.seq_no, sa.module_id, sa.start_date, sa.end_date, sa.allowuntil_date, sa.hide_until_start, sa.override_start, sa.override_end, sa.override_allowuntil, sa.override_hide_until_start from melete_course_module cm,melete_special_access sa where cm.course_id = ? and cm.delete_flag = 0 and cm.archv_flag = 0 and cm.seq_no < ? and cm.module_id = sa.module_id and sa.users like ? and (sa.start_date is null or sa.end_date is null or sa.start_date < sa.end_date) and (sa.start_date is null or sa.allowuntil_date is null or sa.start_date < sa.allowuntil_date) and (sa.end_date is null or sa.allowuntil_date is null or sa.end_date <= sa.allowuntil_date) order by cm.seq_no desc";
			}
			else
			{
				...
				while (accRs.next())
				{
					...
					else
					{
						AccessDates ad = new AccessDates(moduleId, accRs.getTimestamp("start_date"), accRs.getTimestamp("end_date"),
								accRs.getTimestamp("allowuntil_date"), accRs.getBoolean("hide_until_start"), accRs.getBoolean("override_start"), accRs.getBoolean("override_end"),
								accRs.getBoolean("override_allowuntil"), accRs.getBoolean("override_hide_until_start"));
	...
	protected ModuleShdates updateCalendar(String moduleTitle, ModuleShdates moduleshdates1, String courseId) throws Exception
	{
		if (checkCalendar(courseId) == true)
		{
			...
			boolean hideUntilStart = moduleshdates1.getHideUntilStart().booleanValue();
				...
				if (addtoSchedule == true)
				{
					...
					else
					{
						if (startEventId == null)
						{
							...
							if (!hideUntilStart) startEventId = createCalendarEvent(c, startDate, "Opens: " + moduleTitle, desc);
						}
						else
						{
							logger.debug("UPDATING start event for non-nul start date");
							String desc = endDate != null ? "This module opens today and closes " + endDate.toString() : "This module opens today";
							if (!hideUntilStart) startEventId = updateCalendarEvent(c, startEventId, startDate, "Opens: " + moduleTitle, desc);
							else 
							{
								deleteCalendarEvent(c, startEventId);
								startEventId = null;
					...
					if (endDate != null)
					{
						if (endEventId == null)
						{
							...
							if (!hideUntilStart) endEventId = createCalendarEvent(c, endDate, "Closes: " + moduleTitle, desc);
						}
						else
						{
							logger.debug("UPDATING end event for non-null end date");
							String desc = "This module closes today";
							if (!hideUntilStart) endEventId = updateCalendarEvent(c, endEventId, endDate, "Closes: " + moduleTitle, desc);
							else
							{
								deleteCalendarEvent(c, endEventId);
								endEventId = null;
...
class AccessDates
{
	...
	boolean hideUntilStart;
	int moduleId;
	boolean overrideEnd;
	boolean overrideStart;
	boolean overrideAllowUntil;
	boolean overrideHideUntilStart;

	AccessDates(int moduleId, java.sql.Timestamp accStartTimestamp, java.sql.Timestamp accEndTimestamp, java.sql.Timestamp accAllowUntilTimestamp, boolean hideUntilStart, boolean overrideStart, boolean overrideEnd, boolean overrideAllowUntil, boolean overrideHideUntilStart)
	{
	...
	/**
	 * @return hideUntilStart value
	 */
	public boolean isHideUntilStart()
	{
		return this.hideUntilStart;
	}	
	...
	/**
	 * @return override hide until start value
	 */
	public boolean isOverrideHideUntilStart()
	{
		return this.overrideHideUntilStart;
	}	
	...
	/**
	 * @param hideUntilStart
	 *        hide until start value
	 */
	public void setHideUntilStart(boolean hideUntilStart)
	{
		this.hideUntilStart = hideUntilStart;
	}	
	...
	/**
	 * @param overrideHideUntilStart
	 *        override hide until start value
	 */
	public void setOverrideHideUntilStart(boolean overrideHideUntilStart)
	{
		this.overrideHideUntilStart = overrideHideUntilStart;
}
}
